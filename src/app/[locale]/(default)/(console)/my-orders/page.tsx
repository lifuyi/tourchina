...
export default async function () {
  ...
  const columns: TableColumn[] = [
    ...
    {
      name: "download",
      title: t("my_orders.table.download_guide"),
      callback: async (item: any) => {
        // For China Tour Guide product, show download link
        if (item.product_id === "china-guide") {
          return (
            <Link 
              href={`/api/download/${item.order_no}`}
              className="inline-flex items-center gap-1 text-primary hover:underline"
              target="_blank"
            >
              <span>{t("my_orders.table.download_guide")}</span>
            </Link>
          );
        }

        // For subscription products (Stripe), show billing management
        if (
